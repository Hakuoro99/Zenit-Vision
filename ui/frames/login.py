...
def _make_circle_avatar(size, bg, fg, initials, photo=None):
    ...
    # Borde sutil traslúcido para el avatar mapeado al token de borde mineral
    ImageDraw.Draw(border_layer).ellipse(
        (2, 2, r-3, r-3), outline=(255, 255, 255, 30), width=4)
...
def _make_green_hover(size):
    ...
    # Capa verde mineral con opacidad controlada para el estado activo
    ImageDraw.Draw(img).ellipse((0, 0, r-1, r-1), fill=(43, 179, 113, 80))
...
class LoginFrame(ctk.CTkFrame):
    ...
    def render_ui(self):
        ...
        # ── Card con Estilo Mineral (Sin bordes blancos chillones) ────────────
        border_card = ctk.CTkFrame(
            wrapper,
            fg_color=BG1,
            border_width=1,
            border_color=BORDER,   # Borde oscuro mate integrado
            corner_radius=20,
        )
        ...
        self.main_card.pack(padx=50, pady=(32, 36))
        ...
        # ── Título Deportivo/Futurista (Impact/Arial Black) ───────────────────
        ...
        ctk.CTkLabel(self.main_card,
                     ...
                     font=("Arial", 13),
                     text_color=TEXT_GRAY).pack(pady=(0, 20))

        # ── Grid de perfiles ──────────────────────────────────────────────────
        self.profiles_container = ctk.CTkFrame(self.main_card, fg_color="transparent")
        self.profiles_container.pack()
        self.load_local_profiles()

        # ── Divisor ───────────────────────────────────────────────────────────
        div_row = ctk.CTkFrame(self.main_card, fg_color="transparent")
        div_row.pack(pady=(24, 14))
        ctk.CTkFrame(div_row, fg_color=BORDER,
                     height=1, width=100).pack(side="left", padx=(0, 10))
        ctk.CTkLabel(div_row,
                     text="¿No tienes cuenta?",
                     font=("Arial", 11, "bold"),
                     text_color=TEXT_MUTED).pack(side="left")
        ctk.CTkFrame(div_row, fg_color=BORDER,
                     height=1, width=100).pack(side="left", padx=(10, 0))

        # ── Botón registro Estilo Mineral Plano ────────────────────────────────
        reg_btn = ctk.CTkButton(
            ...
            width=240, height=BTN_HEIGHT,
            command=self.go_to_register,
        )
        reg_btn.pack()
        reg_btn.bind("<Enter>", lambda e: reg_btn.configure(text_color="#070809"))
        reg_btn.bind("<Leave>", lambda e: reg_btn.configure(text_color=TEXT_WHITE))
        ...
        if os.path.exists(self.local_users_file):
            with open(self.local_users_file, "r") as f:
                ...
                for user_id, local_info in users.items():
                    cloud_data = self.controller.fb_db.read_record(f"users/{user_id}")
                    if cloud_data:
                        row = i // 3
                        col = i % 3
    ...
    def create_profile_card(self, user_id, data, remember, row, col, idx=0):
        ...
        if data.get("imagen_url"):
            ...
            try:
                photo_pil = Image.open(BytesIO(base64.b64decode(data["imagen_data"])))
            except Exception:
                pass

        img_normal = _make_circle_avatar(AVATAR_SIZE, palette["bg"], palette["fg"], initials, photo_pil)
        ...
        lbl = ctk.CTkLabel(
            ...
            font=("Arial", 11, "bold"), text_color=TEXT_GRAY,
        ...
        def on_enter(e, b=btn, uid=user_id):
            ...
            combined = Image.alpha_composite(
                imgs._light_image.copy(),
                imgs._light_image.copy())
            b.configure(image=ctk.CTkImage(combined, size=(AVATAR_SIZE, AVATAR_SIZE)))

        def on_leave(e, b=btn, uid=user_id):
            imgs = self._hover_imgs.get(uid)
            if imgs:
                b.configure(image=imgs)

        for w in (btn, container, lbl):
            w.bind("<Enter>", on_enter)
            w.bind("<Leave>", on_leave)

    # ── Lógica de Acceso y Modales de Autenticación ───────────────────────────
    ...
    def ask_password(self, user_id):
        ...
        self._center_window(self.pw_window, 380, 290)
        ...
        ctk.CTkLabel(outer, text="Introduce tu contraseña para continuar",
                     font=("Arial", 11),
                     text_color=TEXT_GRAY).pack(pady=(0, 16))
        ...
        self.lbl_pw_error = ctk.CTkLabel(
            ...
            font=("Arial", 11, "bold"))
        self.lbl_pw_error.pack(pady=(4, 0))
    ...
    def _recover_password(self, user_id):
        ...
        rec_win.grab_set()
        self._center_window(rec_win, 400, 270)
        ...
        def send_recovery():
            ...
            if not email:
                lbl_status.configure(text="Ingresa un correo válido", text_color=ERROR_COLOR)
                return
            try:
                self.controller.fb_db.send_password_reset(email)
                lbl_status.configure(text=f"✓ Correo enviado a {email}", text_color=ACCENT_TEXT)
                rec_win.after(2500, rec_win.destroy)
            except Exception as e:
                print(f"Error al enviar recuperación: {e}")
                lbl_status.configure(text="Error al enviar. Verifica el correo.", text_color=ERROR_COLOR)
    ...
    def go_to_dashboard(self):
        ...
        frame = self.controller.frames.get(DashboardFrame)
        if frame:
            frame._build_layout()
